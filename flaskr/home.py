...
def index():
    ...
    soil_moisture = Measurements.query.order_by(Measurements.id.desc()).first().soil_moisture # fetch latest soil moisture

    for pot in flower_pots:
        flower_moist_needed = Plants.query.filter_by(name=pot.plant).first().soil_moisture
